...
const TRAFFIC_CLASS_BITS = new BitArray(0, 8);

const FLOW_LABEL_BITS = new BitArray(0, 20);

const PAYLOAD_LENGTH_BITS = new BitArray(0, 16);

const NEXT_HEADER_BITS = new BitArray(0, 8);

const HOP_LIMIT_BITS = new BitArray(0, 8);
...
export class IPPacketV6 {
    ...
    constructor(source: AddressV6 | BitArray, destination?: AddressV6, protocol?: PROTOCOL, payload?: BitArray, hopLimit = DEFAULT_HOP_LIMIT, flowLabel = FLOW_LABEL_BITS) {
        ...
        let payloadLengthBits = PAYLOAD_LENGTH_BITS.or(new BitArray(Math.ceil(payload.size / 8)));
        this.bits.splice(32, PAYLOAD_LENGTH_BITS.size, payloadLengthBits);    
    }
    ...
    get payload(): BitArray {
        let offset = VERSION_BITS.size + TRAFFIC_CLASS_BITS.size + FLOW_LABEL_BITS.size + PAYLOAD_LENGTH_BITS.size + NEXT_HEADER_BITS.size + HOP_LIMIT_BITS.size +( AddressV6.address_length * 2);
