...
import { Struct, defineStruct } from "../../lib/binary/struct/buffer.struct";
import { UINT8, UINT16, SLICE, UINT64, INT8, INT64, INT16 } from "../../lib/binary/struct/buffer.value-types";
...
describe("Buffer based struct", () => {
    ...
    test("create Struct #1", () => {
        ...
        expect(createdStruct.buf.length).toEqual(128 / 8);
    ...
    test("create Struct #2", () => {
        ...
        expect(createdStruct.buf.length * 8).toEqual(8 * 2 + 16)
        ...
        expect(createdStruct.buf.length * 8).toEqual(8 * 2 + 8)
    });
